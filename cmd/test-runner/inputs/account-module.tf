...
module "cost_center_access_permission_set" {
  ...
  inline_policy_documents = [
    data.aws_iam_policy_document.s3_report_access_policy[each.key].json
  ]
}
...
module "controlling_access_permission_set" {
  ...
  ssoadmin_instance_arn   = local.ssoadmin_instance_arn
  inline_policy_documents = [
    data.aws_iam_policy_document.s3_controlling_acess_policy.json,
    data.aws_iam_policy_document.athena_cur_access_policy.json
  ]
}

data "aws_iam_policy_document" "s3_controlling_acess_policy" {
  statement {
    actions   = [
      "s3:ListAllMyBuckets"
    ]
...
data "aws_iam_policy_document" "athena_cur_access_policy" {
  statement {
